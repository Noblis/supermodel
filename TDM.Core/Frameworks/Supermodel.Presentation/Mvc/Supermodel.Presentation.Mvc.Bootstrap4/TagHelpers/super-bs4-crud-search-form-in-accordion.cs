...
namespace Supermodel.Presentation.Mvc.Bootstrap4.TagHelpers
{
    ...
    public class SuperBs4CRUDSearchFormInAccordionTagHelper : TagHelperDerivedFromHtmlHelperBase
    {
        ...
        {
            output.TagName = null;
            if (PageTitle != null) output.Content.SetHtmlContent(_htmlHelper.Super().Bs4().CRUDSearchFormInAccordion(Expression!.Name, AccordionId, Panels!, PageTitle, Action, Controller, ResetButton));
            else output.Content.SetHtmlContent(_htmlHelper.Super().Bs4().CRUDSearchFormInAccordion(Expression!.Name, AccordionId, Panels!, (IHtmlContent?)null, Action, Controller, ResetButton));
        }
        #endregion
